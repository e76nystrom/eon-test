...
##        print("band, lastBand: ", band, self.lastBand)
##        if debug:
...
##
    ...
    def sendByteList(self):            
        byteList = self.GEORGE[self._step]
...
##                self._CommandAllSlims(f) # GEORGE doesn't need f
...
##                    self._CommandAllSlims(f)
                    self._CommandAllSlims() # GEORGE doesn't need f
                cb.msWait(self.wait)
...
##                self._InputSynth(f) # JGH syndutHook4
    ...
    def NewScanSettings(self, parms):
        ...
        if self._isLogF:
            ...
            z = self._freqs = linspace(fStart, fStop, nSteps+1)
    ...
    def createGEORGE(self):
        print(">>>2976<<< GEORGE in action")
        GEORGE = []
    ...
    ##                self.bitsRBW # JGH line changed for next one
            ...
            GEORGE.append(byteList)
        # print(">>>3071<<< GEORGE's length: ", len(GEORGE))
...
#        msa.baseCal = self.LoadCal(self.baseCalFileName)
            ...
            if itemName == "-":
                ...
                if id == -1:
                    ...
                    if s == 0:
                        ...
                        item = submenu.Append(id, itemName)
                        s -= 1
                        if s == 0:
                            menu.AppendMenu(id, subName, submenu)
                elif id == -2:
                    id = wx.NewId()
                    if s == 0:
                        item = menu.AppendRadioItem(id, itemName)
                    else:
                        item = submenu.AppendRadioItem(id, itemName)
                        s -= 1
                        if s == 0:
                            menu.AppendMenu(id, subName, submenu)
                            
                elif id > 0 and id < 10:
                    #print(">>>14679<<< Next " + str(id) + " items are part of a submenu")
                    subName = itemName
                    submenu = wx.Menu()
                    s = id
                    continue
    ...
    def SubMenu(self, parentMenu, id, text, help, kind, subMenu):
        pass
