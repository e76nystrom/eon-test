...
import os, time, re, string, traceback, wx, warnings, random, thread, threading
...
class MSA:
    ...
    def __init__(self, frame):
        ...
        self.VideoFilters = p.get("VideoFilters", [0.001, 0.1, 1.0, 10.0])
        self.nameVideoSel = p.nameVideoSel = p.get("nameVideoSel", "Narrow") # JGH This is the default mode
        self.bitsVideo = p.indexVideoSel = p.get("indexVideoSel", 2) # JGH This is the default mode
        ...
        self.invDeg = p.get("invDeg", 180.)	# JGH: Default on Startup
...
##        gr.Bind(wx.grid.EVT_GRID_LABEL_LEFT_CLICK, self.OnRBWLabelSel)
        ...
        uFCapDefaults = [0.001, 0.1, 1.0, 10] # Mag defaults
        self.gridVF = gv = wx.grid.Grid(self)
        gv.CreateGrid(4,1)
        for i in range(4):
            gv.SetCellValue(i, 0, "%4.3f" % uFCapDefaults[i])
        ...
        sizerG2B.Add(wx.StaticText(self, -1,  "Control Brd" ), (1, 0), flag=cvl)
...
##        st = wx.StaticText(self, -1,  "Start Mode" )
##        sizerG2B.Add(st, (1, 0), flag=cvl)
##        modeChoices = ["SA w/ SG", "SA w/ TG", "VNA_Tran", "VNA_Refl"]
##        s = p.get("mode", 0)
##        cm = wx.ComboBox(self, -1, modeChoices[s], (0, 0), cwsz, choices=modeChoices, style=wx.CB_READONLY)
##        cm.Enable(True)
##        self.startMode = cm
...
#     return 'we will be back in the basic cal window, or whoever called us
    ...
    def Update(self):
        ...
        msa.fixtureR0 = float(p.fixtureR0)
...
#
    ...
    @classmethod
    def CoaxPropagationGamma(fMHz, VF, K1, K2):
        if fMHz <= 0:
            fMHz = 0.000001
...
#
    ...
    def CoaxPhaseDelayAndLossFromSpecs(coaxSpecs, fMHz):
        ...
        if isErr:
            return True, 0, 0
        phase = Coax.CoaxDelayDegres(fMHz, VF, lenFeet)
...
##        lsVdim = int(p.get("logSplit", 0.8 * self.fVdim))
...
##                VideoFilters.append({Label: [magCap, phaCap]})
                VideoFilters.append({Label: uFcap})
...
#--------------------------------------------------------------------------
    ...
    def RefreshAllParms(self):
        ...
        items = self.sweepMenu.GetMenuItems()
