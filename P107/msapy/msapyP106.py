...
# This one is a consolidated version of the two above, not released yet
...
class MSA_CB:
    ...
    def SendDevBytes(self, bytes, clkMask):
        if self.show:
            print ("SendDevBytes")
        pass
...
if isWin and winUsesParallelPort:
    ...
    # OSX: tell ctypes that the libusb backend is located in the Frameworks dir
...
class MSA_CB_USB(MSA_CB):
    ...
    def FindInterface(self):
        if not self.usbFX2:
            if debug:
                print (">>>>791<<<< MSA_CB_USB: FindInterface XXXXX")
                print ">>>714<<< resdir: ", resdir
            for bus in usb.busses():
                for dev in bus.devices:
                    if dev.idVendor == self.USB_IDVENDOR_CYPRESS and dev.idProduct == self.USB_IDPRODUCT_FX2:
                        odev = dev.open()
                        if debug:
                            print ("Device open ,got odev handle")
...
##                                print ("Interface released")
    ...
    def Flush(self):
        if debug:
            print (">>>>883<<<< MSA_CB_USB:Flush()", len(self._writeFIFO))
        if len(self._writeFIFO) > 0:
            fx2 = self.usbFX2
            if debug:
                print (">>>>789<<<< fx2:  " + str(fx2))
...
class MSA_LO:
    ...
    def __init__(self, id, freq, pllBit, le, fqud, PLLphasefreq, phasepolarity, \
                 appxdds, ddsfilbw, PLLmode, PLLtype):
        ...
        # DDS xtal filter bandwidth (in MHz), at the 3 dB points.
        self.ddsfilbw = ddsfilbw
        # Approx. Phase Detector Frequency (MHz) for PLL. Use .974 when DDS
        # filter is 15 KHz wide PLLphasefreq must be less than the following
        # formula:
        ...
        if msa.spurcheck and not self.PLLmode:
...
##        cb.SetP(1, a)
    ...
    def CreatePLLN(self):
        ...
        self.preselector = (32, 16)[self.PLLmode]
        fcounter = 0 # EON Jan 29, 2014
        
        # CreateNBuffer,
        PLLN = self.PLLtype  # JGH added
        if debug:
            print ("preselector: " + str(self.preselector) + " PLLN: " + PLLN)
    ...
    def Calculate(self, freq):
        ...
        # any 32 bit DDS) is: ddsoutput = base*ddsclock/2^32
        # rounded off to the nearest whole bit
        base = int(round(divSafe(self.ddsoutput * (1<<32), ddsclock)))
        self.DDSbits = base
        if debug:
...
class MSA:
    ...
    def _CommandAllSlims(self, f):
        ...
        if debug:
            print ("      ##### 2116, COMMAND ALL SLIMS #####")
            print (">>>>> 2117, freq=", f, "band=", band, " <<<<<")
...
##                self.bitsRBW # JGH line changed for next one
        ...
        cb.SendDevBytes(bytes, cb.P1_Clk)
    ...
    def InitializeHardware(self):
        ...
        if not hardwarePresent:
            print
            print ("      **** NO HARDWARE PRESENT, USING SYNTHETIC DATA ****")
            print
            # start a generator for synthetic data
        ...
        LO1 = MSA_LO(1, 0., cb.P1_PLL1DataBit, cb.P2_le1, cb.P2_fqud1, \
                     PLL1phasefreq, PLL1phasepol, appxdds1, dds1filbw, PLL1mode, PLL1type)
        LO2 = MSA_LO(2, 1024., cb.P1_PLL2DataBit, cb.P2_le2, 0, PLL2phasefreq, \
                     PLL2phasepol, 0, 0, 0, PLL2type)
        LO3 = MSA_LO(3, 0., cb.P1_PLL3DataBit, cb.P2_le3, cb.P2_fqud3, PLL3phasefreq, \
                     PLL3phasepol, appxdds3, dds3filbw, PLL3mode, PLL3type)
        # JGH change end
        ...
        # JGH ends 2/1/14
        # needs:PLL2phasepolarity,SELT,PLL2
        # Initializes and commands PLL2 R Buffer(s)
        LO2.CommandPLLR()
        # CreatePLL2N
        # ; creates:ncounter,fcounter(0)
        LO2.CreateIntegerNcounter(self.appxLO2, self.masterclock)
        # needs:ncounter,fcounter,PLL2
        # returns with Bcounter,Acounter, and N Bits N0-N23
        LO2.CreatePLLN() 
        # actual LO2 frequency
        LO2.freq = ((LO2.Bcounter*LO2.preselector) + LO2.Acounter + \
                    (LO2.fcounter/16))*LO2.pdf
        # CommandPLL2N
        # needs:N23-N0,control,Jcontrol,port,contclear,LEPLL
        ...
        # needs:rcounter1,PLL1mode,PLL1phasepolarity,SELT,PLL1
...
class ConfigDialog(wx.Dialog): # JGH Heavily modified 1/20/14
    def __init__(self, frame):
        ...
        pllPhafreqChoices = ("0.974","4.000")
        pllModeChoices = ("0: Integ", "1: Fract")
        # JGH addition end
        ...
        cb = wx.CheckBox(self, label='PLL Lock')
        sizerG1.Add(cb, (8, 1), flag=c)
        cb.Bind(wx.EVT_CHECKBOX, self.LockPLLs)
        cb.SetValue(True)
        if cb.GetValue() == True:
        ...
        cmPAR1.Enable(True) # JGH changed to True from false
        ...
        tc.Enable(True) # JGH changed to True from false
        sizerG1.Add(tc, (15, 1), flag=c)

        st = wx.StaticText(self, -1, "Mast Clk (MHz)")
        sizerG1.Add(st, (14, 2), (1, 1), chbt, 10)
        tc = wx.TextCtrl(self, -1, gstr(msa.masterclock), size=tsz) # JGH 2/2/14
        tc.SetBackgroundColour(wx.CYAN)
        self.mastClkBox = tc
        tc.Bind(wx.EVT_SET_FOCUS, self.OnSetFocus)
        sizerG1.Add(tc, (15, 2), flag=c)
        ...
        self.InvDegBox = tc = wx.TextCtrl(self, -1, gstr(p.invDeg), size=tsz) # JGH 2/2/14
        tc.Bind(wx.EVT_SET_FOCUS, self.OnSetFocus)
        tc.SetBackgroundColour(wx.CYAN)
        
        sizerG1.Add(tc, (17, 2), flag=c)
...
##        gr.Bind(wx.grid.EVT_GRID_LABEL_LEFT_CLICK, self.OnRBWLabelSel)
        ...
        cb = wx.CheckBox(self, label='Edit Lock')
        sizerV2A.Add(cb, 0, flag=c)
        sizerV2.Add(sizerV2A, 0, wx.ALL|wx.EXPAND, 5)
        cb.Bind(wx.EVT_CHECKBOX, self.LockRBWs)
        cb.SetValue(True)        
        if cb.GetValue() == True:
            gr.EnableEditing(0)
        else:
...
##        sizerV2B.Add(st, 0, flag=cv)
        ...
        cb = wx.CheckBox(self, label='Edit Lock')
        sizerV2B.Add(cb, 0, flag=c)
        sizerV2.Add(sizerV2B, 0, wx.ALL|wx.EXPAND, 4) 
        cb.Bind(wx.EVT_CHECKBOX, self.LockVFs)
        cb.SetValue(True)
        if cb.GetValue() == True:
            gv.EnableEditing(0)
        else:
    ...
    #--------------------------------------------------------------------------        
    def LockPLLs(self, event):  # JGH added this method
    ...
    # Handle Final RBW Filter ListCtrl items 

    def OnRBWCellSel(self, event): # JGH Tentative: Not implemented yet 1/18/14
        gr =self.gridRBW
        self.rbwRow = event.GetRow() ; self.rbwCol = event.GetCol()
##        gc.SelectRow(self.rbwRow, 0)
        gr.SetCell(self.rbwRow, self.rbwCol)
        self.filterSel = self.rbwRow + 1
        print "Row, Col: ", self.rbwRow, self.rbwCol
...
# Make a calibration file name from a path number, returning (dir, fileName).
...
def CalFileName(pathNum):
    ...
    dir = os.path.join(appdir, "MSA_Info", "MSA_Cal")
...
def CalGenMagFile(file, Madc, Sdb, Sdeg, pathNum, freq, bw, calFreq):
    file.write( \
    ...
    for set in sorted(zip(Madc, Sdb, Sdeg)):
        file.write("%6i %9.3f %8.2f\n" % set)
    file.close()

# Generate a Freq calibration file.

def CalGenFreqFile(file, Fmhz, dbs, calDbm):
    file.write( \
    ...
    for set in sorted(zip(Fmhz, dbs)):
        file.write("%11.6f %9.3f\n" % set)
    file.close()
...
class CalManDialog(wx.Dialog):
    ...
    def SaveIfAllowed(self, parent):
        ...
        if answer == wx.ID_YES:
            dir, fileName = CalFileName(self.pathNum)
            wildcard = "Text (*.txt)|*.txt"
            while True:
                dlg = wx.FileDialog(self, "Save file as...", defaultDir=dir,
    ...
    def OnFileItemSel(self, event):
        ...
        self.pathNum = i
        try:
            dir, fileName = CalFileName(i)
            text = open(os.path.join(dir, fileName), "Ur").read()
...
class TestSetupsDialog(wx.Dialog):
    def __init__(self, frame):
        ...
        self.setupsDir = dir = os.path.join(appdir, "MSA_Info", "TestSetups")
        if not os.path.exists(dir):
            os.makedirs(dir)
    ...
    def OnSave(self, event):
        ...
        id = "%02d.s1p" % (self.setupSel+1)
        frame.SaveCal(msa.bandCal, frame.bandCalFileName[:-4] + id)
        frame.SaveCal(msa.baseCal, frame.baseCalFileName[:-4] + id)
    ...
    def OnLoadWithCal(self, event):
        frame = self.frame
        id = "%02d.s1p" % (self.setupSel+1)
        msa.bandCal = frame.LoadCal(frame.bandCalFileName[:-4] + id)
        msa.baseCal = frame.LoadCal(frame.baseCalFileName[:-4] + id)
        self.OnLoad(event)
...
class TextWindow(wx.Frame):
    def __init__(self, frame, title, pos):
        ...
        if isMac:
            ...
            self.Connect(id, -1, wx.wxEVT_COMMAND_MENU_SELECTED, self.SaveAs)
            mb.Append(menu, "&File")
            menu = wx.Menu()
            mb.Append(menu, "&Edit")
            self.SetMenuBar(mb)
    ...
    def OnActivate(self, event):
        ...
        id = self.saveAsID
        if active:
            frame.fileMenu.Append(id, "Save As...")
            frame.Connect(id, -1, wx.wxEVT_COMMAND_MENU_SELECTED, self.SaveAs)
        else:
            frame.fileMenu.Remove(id)
        event.Skip()
...
#--------------------------------------------------------------------------
    ...
    def Apply(self, event=None):
        ...
        if debug: # JGH Same prints, different location. Will be removed
            print (">>>6930<<< p.RBWFilters[p.indexRBWSel]: ", \
                   p.RBWFilters[p.indexRBWSel])
            print ">>> 6932 <<<< p.rbw: ", p.rbw
            print (">>>6933<<< bitsRBW: ", msa.bitsRBW)
        ...
        if debug:
            print (">>>6945<<< bitsVideo: ", msa.bitsVideo)
...
class RefDialog(wx.Dialog):
    ...
    def OnSelectColorA(self, event):
        ...
        nColors = len(vColors)
        ref = self.refs.get(self.refNum).iColor
...
#--------------------------------------------------------------------------
    ...
    def setDDS(self, freq, P1_DDSDataBit, P2_fqud):
        ddsclock = float(self.masterclockBox.GetValue())
        print (">>>13805<<< ddsclock: ", ddsclock)
...
#        msa.baseCal = self.LoadCal(self.baseCalFileName)
        ...
        if debug:
            print ("")
            print (">>>14244<<< MSASpectrumFrame: Ready to perform StartScan")
            print ("")
        self.StartScan(True)
    ...
    def CreateMenu(self, name, itemList):
        ...
        for itemName, handlerName, id in itemList:
            if itemName == "-":
                ...
                if id == -2:
                    id = wx.NewId()
                    item = menu.Append(id, itemName, itemName, wx.ITEM_RADIO)
                else:
    ...
    def ManageHWConfig(self, event=None): # JGH This method heavily modified 1/20/14
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            p.PLL1mode = int(dlg.cmMOD1.GetValue()[0]) # JGH_001
            p.PLL3mode = int(dlg.cmMOD3.GetValue()[0]) # JGH_001
...
#--------------------------------------------------------------------------
    ...
    def RefreshAllParms(self):
        ...
        specP = self.specP
        if debug:
            print (">>>>14912<<<< Entering RefreshAllParms")
            print ("RefreshAllParms", specP._isReady, self.refreshing)
        ...
        if self.sweepDlg:
            if debug:
                print (">>>>14928<<<< if sweepDlg then UpdateFromPrefs")
    ...
    def LoadPrefs(self, event=None):
        if debug:
            print (">>>15062<<< Entering LoadPrefs")
        if self.prefs:
            self.StopScanAndWait()
        ...
        if debug:
            print (">>>15071<<< Finished LoadPrefs")
    ...
    def ReadCalPath(self):
        ...
        self.StopScanAndWait()
        p = self.prefs
        dir, fileName = CalFileName(p.indexRBWSel+1)
        try:
            f = open(os.path.join(dir, fileName), "Ur")
...
##        p.switchTR = 1   # JGH 11/25/13
    ...
    def SetMode(self, mode):
        ...
        if self.spectrum:
            ...
            vs0 = vScales[0]
            vs0.typeIndex = 1
            vs0.dataType = traceTypesLists[mode][vs0.typeIndex]
            vs0.AutoScale(self)
            vs1 = vScales[1]
            vs1.typeIndex = (0, 2)[mode >= MSA.MODE_VNATran]
            vs1.dataType = traceTypesLists[mode][vs1.typeIndex]
            vs1.AutoScale(self)
...
class CavityFilterTest(wx.Dialog):   
    def __init__(self, frame):
        ...
        if pos == wx.DefaultPosition:
            self.Center()
    #------------------------------------------------------------------------
    def OnCFTest(self, event=None):
        p = self.frame.prefs
##        msaSF = MSASpectrumFrame(self)
        ...
        p.PLL2phasefreq = .1
        if debug:
            print ">>>15574<<< p.PLL2phasefreq", p.PLL2phasefreq
        MSASpectrumFrame.StartScan(False) # JGH True is for HaltAtEnd
    ...
    def CloseCavityFilterTest(self, event=None):
        ...
        p = self.frame.prefs
        cftest = 0
        p.PLL2phasefreq = self.enterPLL2phasefreq
        if debug:
            print ">>>15585<<< p.PLL2phasefreq", p.PLL2phasefreq
